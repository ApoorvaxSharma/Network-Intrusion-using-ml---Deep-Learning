...
ATTACK_DESCRIPTIONS = {
    'dos':    'A Denial-of-Service (DoS) attack floods a machine or network, making it inaccessible by overwhelming it with traffic and shutting out legitimate users.',
...
# ── Wrap everything; on any exception print JSON error to stdout (NOT stderr)
# ── and exit with code 0 so PythonShell reads the output instead of treating it as crash
try:
    ...
    if prot is None or svc is None or flag is None:
        raise ValueError(
            f"Unrecognised value — protocol='{sys.argv[1]}', "
            f"service='{sys.argv[2]}', flag='{sys.argv[3]}'. "
            "Check that the form values match the expected options."
        )
    ...
    votes = sum([
        results['knn']['is_attack'],  results['rf']['is_attack'],
    ...
    # Always print JSON to stdout and exit 0
    print(json.dumps(results))
    sys.exit(0)

except Exception as e:
    # Print the real error as JSON to stdout (NOT stderr) and exit 0
    # so PythonShell captures it instead of treating exit code as the error
    err_detail = traceback.format_exc().strip().splitlines()[-1]
    print(json.dumps({'error': f"{type(e).__name__}: {str(e)}", 'detail': err_detail}))
    sys.exit(0)   # <-- exit 0, NOT 1; let app.js read the JSON error above